create or replace PACKAGE BODY pck_transform IS
   ...
   BEGIN
      ...
      FOR linha IN products_with_problems LOOP
...
PROCEDURE screen_ucs (  p_iteration_key t_tel_iteration.iteration_key%TYPE,
      ...
      CURSOR ucs_with_problems IS
         SELECT u.rowid
         FROM t_data_ucs u 
         JOIN t_lookup_nomeucs n ON (u.nomeuc = n.nomeuc_incorreto)
         WHERE rejected_by_screen='0';
   ...
   -- *************************************************************************************
   -- * GOAL: detect "turnos" with wrongly written nomeuc after                           *
   -- *                         SCREEN_UCS                                                *
   -- * PARAMETERS:                                                                       *
   -- *     p_iteration_key: key of the iteration in which the screen will be run         *
   -- *     p_source_key: key of the source system related to the screen's execution      *
...
PROCEDURE screen_turnos (  p_iteration_key t_tel_iteration.iteration_key%TYPE,
                                            p_source_key t_tel_source.source_key%TYPE,
                                            p_screen_order t_tel_schedule.screen_order%TYPE) IS
    CURSOR turnos_with_problems IS
         SELECT u.rowid
         FROM t_data_turnos u 
         JOIN t_lookup_nomeucs n ON (u.nomeuc = n.nomeuc_incorreto)
         WHERE rejected_by_screen='0';

    i PLS_INTEGER:=0;
    v_screen_name VARCHAR2(30):='screen_turnos';
  BEGIN     
    pck_log.write_log('  Starting SCREEN ["'||UPPER(v_screen_name)||'"] with order #'||p_screen_order||'');

    FOR linha IN turnos_with_problems LOOP
         error_log(v_screen_name,SYSDATE,p_source_key,p_iteration_key,linha.rowid);  
         i:=i+1;
      END LOOP;
  ...
  -- **********************************************************
	...
	BEGIN
		...
		INSERT INTO t_clean_users(id,created_at,turma_id,temporario,hashorariocompleto,preferencia,ramo,regime,role)
		SELECT u.id,u.created_at,u.turma_id,u.temporario,u.hashorariocompleto,u.preferencia,u.ramo,u.regime,u.role  
    FROM t_data_users u
   ...
   BEGIN
      ...
      INSERT INTO t_clean_faltas(id,aula_semana_id,user_id,presente)
      SELECT p.id, p.aula_semana_id,p.user_id,p.presente
   ...
   BEGIN
      ...
      INSERT INTO t_clean_presencas(id,aula_semana_id,user_id,presente)
      SELECT p.id, p.aula_semana_id,p.user_id,p.presente
      FROM t_data_presencas p
      WHERE p.rejected_by_screen='0'
        AND p.presente=1;
   ...
   BEGIN
      ...
      INSERT INTO t_clean_aulas(id,semana,diasemana,horainicio,horafim,sala,dia,mes,ano_civil,turno_id,prof_id,marcou_presenca,num_presencas,
        aula_cancelada)
      SELECT a.id,a.semana,a.diasemana,a.horainicio,a.horafim,a.sala,a.dia,a.mes,a.ano_civil,a.turno_id,a.prof_id,a.marcou_presenca,
        a.num_presencas,a.aula_cancelada
   ...
   BEGIN
      ...
      INSERT INTO t_clean_turnos(id,anolectivo,nomeuc,regimeuc,abrevuc,anouc,semestreuc,ramouc,turnouc,max_alunos,tipoturno,uc_id)
      SELECT t.id,t.anolectivo,
        CASE
            WHEN t.nomeuc=ln.nomeuc_incorreto 
            THEN ln.nomeuc_correto 
            ELSE t.nomeuc 
        END,t.regimeuc,t.abrevuc,t.anouc,t.semestreuc,t.ramouc,t.turnouc,t.max_alunos,t.tipoturno,t.uc_id
   ...
   BEGIN
      ...
      SELECT ucs.ID ID,
            CASE
               WHEN ucs.nomeuc=ln.nomeuc_incorreto 
               THEN ln.nomeuc_correto 
               ELSE ucs.nomeuc 
            END,
            ucs.ABREVUC,
            ...
            ucs.RAMOUC
        FROM T_DATA_UCS ucs
           JOIN T_DATA_CURSO_EI_NEW ce ON (ce.uc LIKE ucs.nomeuc || '%')
           JOIN T_DATA_DEPARTAMENTOS_NEW dp ON ce.DEPARTAMENTO LIKE dp.SIGLA
           JOIN T_DATA_AREA_CIENTIFICA_NEW ac ON ( ce.AREA_CIENTIFICA LIKE substr(ac.sigla, 0, length(ac.sigla)-1))
           LEFT JOIN t_lookup_nomeucs ln ON (ucs.nomeuc LIKE ln.nomeuc_incorreto)
